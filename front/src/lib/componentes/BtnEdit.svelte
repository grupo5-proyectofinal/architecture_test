...
<div class="modal-overlay" on:click={closeModal} role="button" tabindex="0" aria-label="Cerrar modal">
  ...
  <div class="modal-content" role="dialog" aria-labelledby="modal-title" on:click|stopPropagation>
    ...
    <div class="modal-footer">
      {#if !editMode}
        <button class="btn btn-dark rounded-pill" on:click={enableEdit}>Editar</button>
        
      {/if}
      {#if editMode}
        <button class="btn btn-dark rounded-pill" on:click={saveChanges} type="button" aria-label="Editar pool">
          Guardar cambios
        </button>
      {/if}
    </div>
